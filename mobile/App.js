...
function LoginScreen({ navigation }) {
  ...
  const login = async () => {
    ...
    try {
      ...
      navigation.replace('Dashboard', { user: res.data.data.user });
    } catch (e) {
...
function RegisterScreen({ navigation }) {
  ...
  return (
    <KeyboardAvoidingView style={s.c} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      ...
      <TextInput style={s.input} placeholder="Password" placeholderTextColor="#666" value={form.password} onChangeText={v=>setForm(p=>({...p,password:v}))} secureTextEntry />
